...
class SyniAgent {
  ...
  ///
  /// Cloud chat only needs a persona + a configured `SyniCloudConfig`;
  /// it doesn't run the on-device engine. Hosts that pick
  /// [SyniExecutionMode.cloudOnly] can call this once instead of
  /// downloading a model just to bind a persona. Local / localFirst
  /// modes still require [install] to have completed (it loads the
  /// engine), and [install] continues to bind the persona itself.
  ///
  /// Idempotent — re-binding the same persona is harmless. Replacing
  /// the persona while a model is installed is allowed; the persona
  /// you bind last is the one chat methods use.
  ...
  /// Permanently removes [model] from the device: frees the engine + worker
  /// isolate, then deletes the GGUF and its sibling `tokenizer.json` from
  /// disk, reclaiming the ~1.1 GB the install occupies. Resets [installState]
  /// to [SyniNotInstalled] so a subsequent [install] re-downloads.
  ///
  /// Unlike [uninstall] (which only frees the in-memory engine), this is a
  /// genuine storage-reclaiming delete. Returns the number of bytes freed.
  /// Throws [SyniInstallException] if a model file cannot be removed; the
  /// engine is still disposed and state is left as [SyniNotInstalled] in that
  /// case so the agent isn't stuck.
  ...
  /// Gate for chat methods: always require a bound persona; require an
  /// installed local engine *only* when the request would actually run
  /// on device. Cloud-only chat needs no model — `bindPersona` is
  /// enough — so it skips the install check.
  SyniPersona _requirePersona({required SyniExecutionMode mode}) {
    final p = _persona;
    if (p == null) {
      throw StateError(
        'No persona bound. Call install() (local / localFirst) or '
        'bindPersona() (cloudOnly) first.',
      );
    }
    if (mode != SyniExecutionMode.cloudOnly) {
      if (currentState is! SyniInstalled) {
