#!/usr/bin/env bash
# Mirror syni-core-spec/personas/{prod,research} into syni-flutter's
# bundled assets so SyniSpecPersona.load() can find them.
#
# Usage:
#   bash tool/sync_personas.sh
#   SPEC_PERSONA_SRC=/path/to/syni-core-spec/personas bash tool/sync_personas.sh
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
SRC="${SPEC_PERSONA_SRC:-$ROOT/../syni-core-spec/personas}"
DST="$ROOT/assets/personas"

if [ ! -d "$SRC" ]; then
  echo "syni-core-spec personas not found at $SRC — set SPEC_PERSONA_SRC=..." >&2
  exit 1
fi
